...
def load_data_into_db():
    ...
    df_ls = pd.read_csv(DATA_DIRECTORY+'leadscoring_test.csv')

    df_ls['total_leads_droppped'] = df_ls['total_leads_droppped'].fillna(0)
    df_ls['referred_lead'] = df_ls['referred_lead'].fillna(0)

    df_ls.to_sql(name='loaded_data', con=conn, if_exists='replace', index=False)
...
def map_city_tier():
    ...
    df_ls = pd.read_sql('select * from loaded_data', conn)

    df_ls["city_tier"] = df_ls["city_mapped"].map(
        city_tier_mapping)
    df_ls["city_tier"] = df_ls["city_tier"].fillna(3.0)
    df_ls = df_ls.drop(['city_mapped'], axis=1)

    df_ls.to_sql(name='city_tier_mapped', con=conn,
...
def map_categorical_vars():
    ...
    df_ls = pd.read_sql('select * from city_tier_mapped', conn)

    new_df = df_ls[~df_ls['first_platform_c'].isin(
        list_platform)]
    # replace the value of these levels to others
    new_df['first_platform_c'] = "others"
    old_df = df_ls[df_ls['first_platform_c'].isin(
